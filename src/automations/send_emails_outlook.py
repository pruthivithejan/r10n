    ...
    try:
        with open(recipients_file, encoding="utf-8") as f:
            ...
            for line_num, row in enumerate(reader, 1):
                ...
                if len(row) >= 3:
                    recipients.append(
                        {"name": row[0].strip(), "email": row[1].strip(), "file": row[2].strip()}
                    )
                else:
...
def send_outlook_emails_with_attachments(
    ...
    try:
        ...
        with smtplib.SMTP(config["smtp_server"], config["smtp_port"]) as server:
            ...
            for i, recipient in enumerate(recipients, 1):
                try:
                    ...
                    if os.path.exists(file_path):
                        with open(file_path, "rb") as f:
                            ...
                            part["Content-Disposition"] = (
                                f'attachment; filename="{recipient["file"]}"'
                            )
                            msg.attach(part)
                    ...
                    error_msg = f"Failed to send to {recipient['email']}: {e!s}"
                    print(f"  ✗ {error_msg}")
        ...
        raise Exception(f"SMTP connection error: {e!s}")
    ...
    return {"total": len(recipients), "sent": sent_count, "failed": failed_count, "errors": errors}


def send_from_file(
    recipients_file="data/outlook/recipients.txt",
