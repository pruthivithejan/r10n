...
Return only the corrected content without any additional comments or explanations."""
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a professional proofreader. Your job is to fix grammar, spelling, and punctuation errors while preserving the original writing style and content.",
                    },
            ...
            print(f"❌ Error proofreading content: {e!s}")
    ...
    def process_blog_post(
        self, content: str, custom_metadata: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"❌ Error processing blog post: {e!s}")
            return {
                "filename": None,
                "content": None,
                "metadata": None,
    ...
    def save_mdx_file(self, filename: str, content: str) -> bool:
        ...
        try:
            ...
            print(f"❌ Error saving MDX file: {e!s}")
            return False

    def process_from_file(
        self, input_file: str, custom_metadata: Dict[str, Any] = None
    ) -> Dict[str, Any]:
            ...
            if result["success"]:
                ...
                if self.save_mdx_file(result["filename"], result["content"]):
                    result["output_path"] = str(
                        Path(self.config.output_directory) / result["filename"]
                    )
                    print("🎉 Blog post processed successfully!")
...
def generate_blog_mdx(
    ...
    try:
        ...
        print(f"❌ Error in blog generation: {e!s}")
        return {"success": False, "error": str(e)}
