...
def contacts(interactive, config, input, output, prefix):
    ...
    if interactive:
        ...
        input = input or Prompt.ask(
            "\n[cyan]Input file path[/]",
            default="workspace/inputs/contacts/numbers.txt"
        )
        ...
        # Get prefix
        prefix = prefix or Prompt.ask(
            "[cyan]Contact name prefix[/]",
            default="Contact"
        )
...
def email(interactive, config, recipients, body, type):
    ...
    if interactive:
        display_header(
            "Email Automation",
            "Send bulk or personalized emails with templates"
        )
        
        # Choose email type
        email_type = type or Prompt.ask(
            "\n[cyan]Email type[/]",
            choices=["bulk", "personalized"],
            default="bulk"
        )
        ...
        if cfg:
            display_config(
                {k: v for k, v in cfg.items() if "password" not in k.lower()},
                "Email Configuration"
            )
        
        # Get recipients file
        recipients = recipients or Prompt.ask(
            "\n[cyan]Recipients file[/]",
            default="workspace/inputs/email/recipients.csv"
        )
        
        # Get email body template
        body = body or Prompt.ask(
            "[cyan]Email body template[/]",
            default="workspace/inputs/email/email_template.txt"
        )
...
def certificates(interactive, config, recipients, template, output):
    ...
    if interactive:
        ...
        template = template or Prompt.ask(
            "\n[cyan]PDF template file[/]",
            default="templates/certificates/template.pdf"
        )
        ...
        recipients = recipients or Prompt.ask(
            "[cyan]Recipients file[/]",
            default="workspace/inputs/certificates/recipients.txt"
        )
        
        # Get output directory
        output = output or Prompt.ask(
            "[cyan]Output directory[/]",
            default="workspace/outputs/certificates"
        )
...
def images(interactive, input, output, quality, max_size, prefix):
    ...
    if interactive:
        ...
        # Get input directory
        input = input or Prompt.ask(
            "\n[cyan]Input directory[/]",
            default="workspace/inputs/images"
        )
        ...
        images = [f for f in Path(input).iterdir() 
        ...
        output = output or Prompt.ask(
            "[cyan]Output directory[/]",
            default="workspace/outputs/images"
        )
        
        # Get quality
        quality = quality or IntPrompt.ask(
            "[cyan]Image quality (1-100)[/]",
            default=85
        )
        
        # Get max size
        max_size = max_size or float(Prompt.ask(
            "[cyan]Maximum file size (MB)[/]",
            default="1.0"
        ))
        
        # Get prefix
        prefix = prefix or Prompt.ask(
            "[cyan]File name prefix[/]",
            default="img"
        )
...
def blog(interactive, input, config, title, author, tags):
    ...
    if interactive:
        display_header(
            "Blog MDX Generation",
            "Create well-structured MDX blog posts with AI assistance"
        )
        
        # Get input file
        input = input or Prompt.ask(
            "\n[cyan]Input blog content file[/]",
            default="workspace/inputs/blog/post.txt"
        )
        ...
        author = author or Prompt.ask(
            "[cyan]Author name[/]",
            default=cfg.get('default_author', 'Your Name')
        )
...
def status():
    ...
    # Check configs
    config_files = list(Path("workspace/configs").glob("*.json")) if Path("workspace/configs").exists() else []
    if config_files:
